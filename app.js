...
async function loadDailyOperationsReport_(){
 ...
 const [today,work,lf,maintenanceReport,maintenanceBoard,pmBoard,hotelPerformanceResult,...shiftResults]=await Promise.all([apiPost({action:'getToday',businessDate:date},45000),apiPost({action:'getWorkBoard'},45000),apiPost({action:'getLostFound',sessionId:sid},45000),apiPost({action:'getMaintenanceReport',sessionId:sid,businessDate:date},45000).catch(()=>({ok:false})),apiPost({action:'getMaintenanceBoard',sessionId:sid,businessDate:date,worker:currentUser.name},45000).catch(()=>({ok:false})),apiPost({action:'getPmBoard',sessionId:sid},45000).catch(()=>({ok:false})),apiPost({action:'getHotelPerformance',businessDate:date},45000).catch(()=>({ok:false})),apiPost({action:'getShiftOperations',sessionId:sid,businessDate:date},45000)]);
 ...
 const lfItems=lf.ok?(lf.items||[]):[],lfStatus=x=>String(x.status||'').toUpperCase().replace(/[^A-Z]+/g,'_'),lfIdDate=x=>{const m=String(x.id||'').match(/^LF-(\d{4})(\d{2})(\d{2})-/);return m?m[1]+'-'+m[2]+'-'+m[3]:''},lfBusinessDate=x=>{const explicit=x.businessDate||x.business_date||'';return explicit?dorDateKey_(explicit):(lfIdDate(x)||dorDateKey_(x.foundAt))},activeBd=date===relayBusinessDate,nextCalendarDay=(()=>{const d=new Date(date+'T12:00:00');d.setDate(d.getDate()+1);return [d.getFullYear(),String(d.getMonth()+1).padStart(2,'0'),String(d.getDate()).padStart(2,'0')].join('-')})(),logged=lfItems.filter(x=>{const bd=lfBusinessDate(x);return bd===date||(activeBd&&!x.businessDate&&!x.business_date&&lfIdDate(x)===nextCalendarDay)}),due=lfItems.filter(x=>dorDateKey_(x.dispositionDate)===date&&!['RETURNED','RETURNED_PICKED_UP','SHIPPED','DISPOSED','DISPOSED_DONATED'].includes(lfStatus(x)));
