...
async function loadInspectorPerformanceReport_(){
 const status=document.getElementById('iprStatus'),select=document.getElementById('iprInspector');if(!status||!select)return;const eligibleCurrent=['Ryan Kelly','Ryland Early'];if(!select.dataset.ready){select.innerHTML='<option value="">Select inspector…</option>'+eligibleCurrent.map(n=>'<option value="'+dorEsc_(n)+'">'+dorEsc_(n)+'</option>').join('');select.dataset.ready='1'}const r=iprRange_();if(!select.value){status.textContent='SELECT INSPECTOR';return}status.textContent='LOADING';
 try{
  ...
  const reportNames=ins.map(iprInspectorName_).filter(Boolean),names=[...new Set([...eligibleCurrent,...reportNames])],old=select.value;select.innerHTML='<option value="">Select inspector…</option>'+names.map(n=>'<option value="'+dorEsc_(n)+'">'+dorEsc_(n)+'</option>').join('');if(names.includes(old))select.value=old;const name=select.value;if(!name){status.textContent='SELECT INSPECTOR';return}
...
/* RELAY Preventive Maintenance Report — live PM rotation */
(function(){
 const report=document.getElementById('roomPmReport'),library=document.getElementById('reportLibrary'),open=document.getElementById('openRoomPmReport');if(!report||!open)return;
 const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
 const key=v=>dorDateKey_(v),fmt=v=>{const k=key(v);if(!k)return '—';const d=new Date(k+'T12:00:00');return isNaN(d)?k:d.toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'})};
 const mins=x=>{const n=Number(x?.activeMinutes??x?.active_minutes);return Number.isFinite(n)?n:null};
 const addDays=(v,n)=>{const k=key(v);if(!k)return '';const d=new Date(k+'T12:00:00');d.setDate(d.getDate()+n);return [d.getFullYear(),String(d.getMonth()+1).padStart(2,'0'),String(d.getDate()).padStart(2,'0')].join('-')};
 const inRange=(x,range,today)=>{if(range==='ALL')return true;const d=key(x.businessDate||x.business_date||x.completedAt||x.completed_at);if(!d)return false;const cutoff=new Date(today+'T12:00:00');cutoff.setDate(cutoff.getDate()-(Number(range)-1));return d>=key(cutoff)&&d<=today};
 function render(board,warranty){
  const today=key(board.businessDate)||key(new Date()),schedule=board.schedule||[],allHistory=board.history||[],range=document.getElementById('pmrRange').value,history=allHistory.filter(x=>inRange(x,range,today));
  const due=schedule.filter(x=>x.dueDate===today),over=schedule.filter(x=>x.dueDate<today),soon=schedule.filter(x=>x.dueDate>today&&x.dueDate<=addDays(today,14)),current=schedule.filter(x=>x.dueDate>addDays(today,14));
  const timed=history.map(mins).filter(x=>x!==null),avg=timed.length?Math.round(timed.reduce((a,b)=>a+b,0)/timed.length):null;
  document.getElementById('pmrKpis').innerHTML=[['ROOMS COMPLETED',history.length,'PMs in selected period',''],['ON SCHEDULE',current.length+soon.length,'rooms beyond today','current'],['DUE NOW',due.length,'due '+fmt(today),'due'],['OVERDUE',over.length,'past scheduled date','overdue'],['AVG PM TIME',avg===null?'—':avg+'m',timed.length?'tracked active time':'time not yet recorded','time']].map(x=>'<article class="pmr-kpi '+x[3]+'"><small>'+x[0]+'</small><strong>'+x[1]+'</strong><span>'+x[2]+'</span></article>').join('');
  const total=schedule.length||114,healthy=current.length+soon.length,pct=total?Math.round(healthy/total*100):0;document.getElementById('pmrHealthPct').textContent=pct+'%';document.getElementById('pmrHealthBar').style.width=pct+'%';document.getElementById('pmrHealthLegend').innerHTML=[['CURRENT',current.length],['DUE SOON',soon.length],['DUE NOW',due.length],['OVERDUE',over.length]].map(x=>'<div><strong>'+x[1]+'</strong><small>'+x[0]+'</small></div>').join('');
  const sorted=schedule.slice().sort((a,b)=>String(a.dueDate).localeCompare(String(b.dueDate))).filter(x=>x.dueDate>=today).slice(0,12);document.getElementById('pmrUpcoming').innerHTML=sorted.length?sorted.map(x=>'<div class="pmr-list-row"><span><strong>Room '+esc(x.room)+'</strong><small>'+(x.session?'In progress • '+esc(x.session.worker||''):'Available from PM rotation')+'</small></span><b>'+fmt(x.dueDate)+'</b></div>').join(''):'<div class="pmr-empty">No upcoming rooms found.</div>';
  const totalMin=timed.reduce((a,b)=>a+b,0),fast=history.filter(x=>mins(x)!==null).sort((a,b)=>mins(a)-mins(b))[0],slow=history.filter(x=>mins(x)!==null).sort((a,b)=>mins(b)-mins(a))[0];document.getElementById('pmrTime').innerHTML=[['TOTAL PM TIME',timed.length?(Math.floor(totalMin/60)+'h '+Math.round(totalMin%60)+'m'):'—'],['AVERAGE / ROOM',avg===null?'—':avg+' min'],['FASTEST',fast?mins(fast)+' min • Room '+esc(fast.room):'—'],['LONGEST',slow?mins(slow)+' min • Room '+esc(slow.room):'—']].map(x=>'<div class="pmr-stat-row"><span>'+x[0]+'</span><b>'+x[1]+'</b></div>').join('');
  const wo=history.reduce((n,x)=>n+Number(x.workOrders||x.work_orders||0),0),roomsWith=history.filter(x=>Number(x.workOrders||x.work_orders||0)>0).length;document.getElementById('pmrDeficiencies').innerHTML=[['Rooms PM’d',history.length],['Rooms with deficiencies',roomsWith],['Maintenance work orders generated',wo],['Warranty items from PM',(warranty||[]).filter(x=>{const source=String(x.source||x.sourceType||x.source_type||x.origin||x.createdFrom||x.created_from||'').toUpperCase(),pmId=String(x.pmSessionId||x.pm_session_id||x.pmCompletionId||x.pm_completion_id||'').trim(),d=key(x.businessDate||x.business_date||x.reportedAt||x.reported_at);return (pmId||source.includes('PM')||source.includes('PREVENTIVE'))&&(!d||inRange({businessDate:d},range,today))}).length]].map(x=>'<div class="pmr-stat-row"><span>'+x[0]+'</span><b>'+x[1]+'</b></div>').join('')+'<p>Deficiency category detail will populate as PM completion history exposes item-level results.</p>';
  document.getElementById('pmrHistoryCount').textContent=history.length+' completed';document.getElementById('pmrHistoryBody').innerHTML=history.length?history.slice().sort((a,b)=>String(b.businessDate).localeCompare(String(a.businessDate))).map(x=>'<tr><td>'+fmt(x.businessDate||x.completedAt)+'</td><td><b>'+esc(x.room||'—')+'</b></td><td>'+esc(x.completedBy||x.completed_by||'—')+'</td><td>'+(mins(x)===null?'—':mins(x)+' min')+'</td><td>'+Number(x.workOrders||x.work_orders||0)+'</td><td>'+fmt(addDays(x.businessDate||x.completedAt,84))+'</td></tr>').join(''):'<tr><td colspan="6" class="pmr-empty">No completed PMs in this range.</td></tr>';
  const itemCounts=ROOM_PM_TASKS.map((t,i)=>{let count=0;history.forEach(x=>{const items=x.items||x.pmItems||x.itemResults||[];if(items.some(y=>Number(y.index??y.itemIndex)===i&&y.status))count++});return [t[0],count]});const hasItems=itemCounts.some(x=>x[1]);document.getElementById('pmrChecklist').innerHTML=hasItems?itemCounts.map(x=>'<div class="pmr-check-row"><span>'+esc(x[0])+'</span><b>'+x[1]+' / '+history.length+'</b></div>').join(''):'<div class="pmr-empty">Checklist item history is not exposed in the current report feed yet. Relay will not invent completion counts.</div>';
  const warr=(warranty||[]).filter(x=>{const source=String(x.source||x.sourceType||x.source_type||x.origin||x.createdFrom||x.created_from||'').toUpperCase(),pmId=String(x.pmSessionId||x.pm_session_id||x.pmCompletionId||x.pm_completion_id||'').trim(),d=key(x.businessDate||x.business_date||x.reportedAt||x.reported_at);return (pmId||source.includes('PM')||source.includes('PREVENTIVE'))&&(!d||inRange({businessDate:d},range,today))});document.getElementById('pmrFlow').innerHTML=[['PMs completed',history.length],['Deficiencies / work orders',wo],['Warranty items identified',warr.length]].map((x,i)=>'<div class="pmr-flow-line"><span>'+x[0]+'</span><strong>'+x[1]+'</strong></div>'+(i<2?'<div class="pmr-flow-arrow">↓</div>':'')).join('')+'<p>Replacement value protected will be added when value tracking is captured in RELAY.</p>';
 }
 async function load(){document.getElementById('pmrHistoryBody').innerHTML='<tr><td colspan="6" class="pmr-empty">Loading live PM rotation…</td></tr>';try{const sid=localStorage.getItem('relaySessionId'),[board,warranty]=await Promise.all([apiPost({action:'getPmBoard',sessionId:sid},45000),apiPost({action:'getWarrantyReport',sessionId:sid},45000).catch(()=>({ok:false,items:[]}))]);if(!board.ok)throw new Error(board.reason||board.error||'Could not load PM board');render(board,warranty.items||[])}catch(e){document.getElementById('pmrHistoryBody').innerHTML='<tr><td colspan="6" class="pmr-empty">Could not load PM report: '+esc(e.message)+'</td></tr>'}}
 open.addEventListener('click',()=>{library.hidden=true;document.querySelectorAll('#reportsView>section').forEach(x=>x.hidden=true);report.hidden=false;load()});document.getElementById('backFromRoomPmReport').addEventListener('click',()=>{report.hidden=true;library.hidden=false});document.getElementById('printRoomPmReport').addEventListener('click',()=>window.print());document.getElementById('pmrRange').addEventListener('change',load);document.querySelectorAll('[data-view="Reports"]').forEach(b=>b.addEventListener('click',()=>{report.hidden=true}));
})();
...
(function(){
 ...
 const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));const dk=v=>dorDateKey_(v),fmt=v=>{const k=dk(v);if(!k)return '—';return new Date(k+'T12:00:00').toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'})};const minutes=x=>{const n=Number(x.activeMinutes??x.active_minutes??x.minutes??x.durationMinutes??x.duration_minutes);return Number.isFinite(n)?n:null};const addMonth=v=>{const k=dk(v);if(!k)return '';const d=new Date(k+'T12:00:00');d.setMonth(d.getMonth()+1);return [d.getFullYear(),String(d.getMonth()+1).padStart(2,'0'),String(d.getDate()).padStart(2,'0')].join('-')};
 function render(b){const today=dk(b.businessDate)||dk(new Date()),schedule=b.schedule||[],history=b.history||[],month=document.getElementById('fprMonth').value||today.slice(0,7),mh=history.filter(x=>dk(x.businessDate||x.completedAt).slice(0,7)===month),doneAssets=new Set(mh.map(x=>String(x.assetId||x.asset_id))),over=schedule.filter(x=>dk(x.dueDate)<today),due=schedule.filter(x=>dk(x.dueDate)===today),on=schedule.filter(x=>dk(x.dueDate)>today),timed=mh.map(minutes).filter(x=>x!==null),total=timed.reduce((a,b)=>a+b,0),avg=timed.length?Math.round(total/timed.length):null;
 document.getElementById('fprKpis').innerHTML=[['ASSETS',schedule.length||4,'3 Mechanical + 1 AHU',''],['COMPLETED THIS MONTH',doneAssets.size,'of '+(schedule.length||4)+' assets','complete'],['DUE NOW',due.length,'scheduled today','due'],['OVERDUE',over.length,'past due date','overdue'],['TRACKED TIME',timed.length?total+'m':'—',avg===null?'No timed completions':'Avg '+avg+'m / completion','time']].map(x=>'<article class="fpr-kpi '+x[3]+'"><small>'+x[0]+'</small><strong>'+x[1]+'</strong><span>'+x[2]+'</span></article>').join('');
 document.getElementById('fprStatusLine').textContent=(over.length?'ATTENTION REQUIRED':due.length?'DUE TODAY':'ON TRACK')+' • '+schedule.length+' assets';document.getElementById('fprAssets').innerHTML=schedule.length?schedule.map(s=>{const latest=history.filter(x=>String(x.assetId||x.asset_id)===String(s.assetId||s.asset_id)).sort((a,b)=>String(b.completedAt||b.businessDate).localeCompare(String(a.completedAt||a.businessDate)))[0],status=dk(s.dueDate)<today?'OVERDUE':dk(s.dueDate)===today?'DUE TODAY':'ON TRACK',cl=status==='OVERDUE'?'overdue':status==='DUE TODAY'?'due':'';return '<article class="fpr-asset '+cl+'"><strong>'+esc(s.assetName||s.asset_name||s.assetId||'Filter Asset')+'</strong><span>'+status+'</span><small>Last completed: '+fmt(latest?.businessDate||latest?.completedAt)+'</small><small>Next due: '+fmt(s.dueDate)+'</small></article>'}).join(''):'<div class="fpr-empty">No filter PM schedule returned.</div>';
 document.getElementById('fprMonthly').innerHTML=[['Report month',new Date(month+'-01T12:00:00').toLocaleDateString(undefined,{month:'long',year:'numeric'})],['Assets completed',doneAssets.size+' / '+(schedule.length||4)],['Completion records',mh.length],['Monthly compliance',(schedule.length?Math.round(doneAssets.size/schedule.length*100):0)+'%']].map(x=>'<div class="fpr-stat"><span>'+x[0]+'</span><b>'+x[1]+'</b></div>').join('');document.getElementById('fprTime').innerHTML=[['Total tracked time',timed.length?total+' min':'—'],['Average / completion',avg===null?'—':avg+' min'],['Timed completions',timed.length],['Completed by',Array.from(new Set(mh.map(x=>x.completedBy||x.completed_by||x.worker).filter(Boolean))).join(', ')||'—']].map(x=>'<div class="fpr-stat"><span>'+x[0]+'</span><b>'+esc(x[1])+'</b></div>').join('');
...
/* RELAY Labor & Productivity Report — weekly ADP + tracked activity */
(function(){
 const report=document.getElementById('laborProductivityReport'),library=document.getElementById('reportLibrary'),open=document.getElementById('openLaborProductivityReport');if(!report||!open)return;
 let users_=[],saved_=[];
 const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
 const n=v=>{const x=Number(v);return Number.isFinite(x)?x:0};
 const date=d=>[d.getFullYear(),String(d.getMonth()+1).padStart(2,'0'),String(d.getDate()).padStart(2,'0')].join('-');
 const week=()=>{const e=new Date((document.getElementById('lprWeekEnd').value||date(new Date()))+'T12:00:00'),s=new Date(e);s.setDate(e.getDate()-6);return {start:date(s),end:date(e)}};
 const fmtMin=m=>{m=Math.max(0,Math.round(n(m)));return m>=60?Math.floor(m/60)+'h '+String(m%60).padStart(2,'0')+'m':m+'m'};
 const mins=x=>{for(const k of ['activeMinutes','active_minutes','minutes','durationMinutes','duration_minutes','cleaningMinutes','timeSpentMinutes']){const z=Number(x?.[k]);if(Number.isFinite(z)&&z>0)return z}const a=x?.startedAt||x?.started_at,b=x?.completedAt||x?.completed_at||x?.readyAt||x?.ready_at;if(a&&b){const z=(new Date(b)-new Date(a))/60000;if(Number.isFinite(z)&&z>0)return z}return 0};
 const stat=(label,value)=>'<div class="lpr-stat"><span>'+label+'</span><b>'+value+'</b></div>';
 function defaultSunday_(){const d=new Date(),day=d.getDay();d.setDate(d.getDate()-day);return date(d)}
 async function loadDays_(w){const dates=[];for(let d=new Date(w.start+'T12:00:00');d<=new Date(w.end+'T12:00:00');d.setDate(d.getDate()+1))dates.push(date(d));const states=[],works=[];for(const d of dates){const [a,b]=await Promise.all([apiPost({action:'getToday',businessDate:d},45000).catch(()=>({ok:false})),apiPost({action:'getWorkBoard',businessDate:d},45000).catch(()=>({ok:false}))]);if(a.ok)states.push(a);if(b.ok)works.push(b)}return {states,works}}
 async function load_(){
   const sid=localStorage.getItem('relaySessionId')||'',w=week(),status=document.getElementById('lprHoursStatus');status.textContent='Loading '+w.start+' through '+w.end+'…';
   try{
     const [u,h,days,mr,pm,fp]=await Promise.all([
       apiPost({action:'getLaborUsers',sessionId:sid},45000),
       apiPost({action:'getLaborHours',sessionId:sid,weekStart:w.start,weekEnd:w.end},45000),
       loadDays_(w),
       apiPost({action:'getMaintenanceReport',sessionId:sid,businessDate:w.end},45000).catch(()=>({ok:false})),
       apiPost({action:'getPmBoard',sessionId:sid},45000).catch(()=>({ok:false})),
       apiPost({action:'getFilterPmBoard',sessionId:sid},45000).catch(()=>({ok:false}))
     ]);
     if(!u.ok)throw new Error(u.reason||u.error||'Labor users unavailable');if(!h.ok)throw new Error(h.reason||h.error||'Labor hours unavailable');
     users_=u.users||[];saved_=h.rows||[];const byId=new Map(saved_.map(x=>[String(x.userId),n(x.actualHours)]));
     document.getElementById('lprHoursBody').innerHTML=users_.length?users_.map(x=>'<tr><td><b>'+esc(x.name)+'</b></td><td>'+esc(x.department)+'</td><td>'+esc(x.role)+'</td><td><input class="lpr-hours" data-user="'+esc(x.userId)+'" inputmode="decimal" type="number" min="0" step="0.01" value="'+(byId.has(String(x.userId))?byId.get(String(x.userId)):'')+'" placeholder="0.00"></td></tr>').join(''):'<tr><td colspan="4">No active associates returned.</td></tr>';
     const paid=saved_.reduce((a,x)=>a+n(x.actualHours),0),dept={};saved_.forEach(x=>{dept[x.department]=(dept[x.department]||0)+n(x.actualHours)});
     const clean=[],ins=[],issues=[],side=[],sideSessions=[];days.states.forEach(x=>{(x.cleaningSessions||[]).forEach(y=>clean.push(y));(x.inspections||[]).forEach(y=>ins.push(y));(x.inspectionIssues||[]).forEach(y=>issues.push(y))});days.works.forEach(x=>{(x.sideWork||[]).forEach(y=>side.push(y));(x.sideWorkSessions||[]).forEach(y=>sideSessions.push(y))});
     const finished=clean.filter(x=>['COMPLETE','READY_FOR_INSPECTION'].includes(String(x.status||'').toUpperCase())),cleanM=finished.reduce((a,x)=>a+mins(x),0),sideM=sideSessions.reduce((a,x)=>a+mins(x),0),reworkM=issues.reduce((a,x)=>a+n(x.reworkMinutes||x.rework_minutes||x.correctionMinutes||x.correction_minutes),0);
     const maint=(mr.ok?(mr.maintenance||[]):[]),maintDone=maint.filter(x=>['COMPLETE','COMPLETED','RESOLVED','CLOSED'].includes(String(x.status||'').toUpperCase())||x.completedAt||x.completed_at),maintM=maintDone.reduce((a,x)=>a+mins(x),0);
     const pmHist=(pm.ok?(pm.history||[]):[]).filter(x=>{const d=dorDateKey_(x.businessDate||x.completedAt);return d>=w.start&&d<=w.end}),pmM=pmHist.reduce((a,x)=>a+mins(x),0),filterHist=(fp.ok?(fp.history||[]):[]).filter(x=>{const d=dorDateKey_(x.businessDate||x.completedAt);return d>=w.start&&d<=w.end}),filterM=filterHist.reduce((a,x)=>a+mins(x),0);
     const tracked=cleanM+sideM+reworkM+maintM+pmM+filterM,visibility=paid?Math.round((tracked/60)/paid*100):0;
     document.getElementById('lprKpis').innerHTML=[['PAID HOURS',paid?paid.toFixed(2):'—','ADP actual hours','paid'],['RELAY TRACKED',tracked?fmtMin(tracked):'—','captured activity','tracked'],['ROOMS CLEANED',new Set(finished.map(x=>String(x.room||'')).filter(Boolean)).size,'completed cleaning','rooms'],['MAINT. TASKS',maintDone.length+pmHist.length+filterHist.length,'reactive + PM','maint'],['TIME VISIBLE',paid?visibility+'%':'—','tracked ÷ paid hours','visibility']].map(x=>'<article class="lpr-kpi '+x[3]+'"><small>'+x[0]+'</small><strong>'+x[1]+'</strong><span>'+x[2]+'</span></article>').join('');
     const deptNames=['Housekeeping','Front Desk','Maintenance','Management / Inspection'];document.getElementById('lprDepartments').innerHTML=deptNames.map(d=>stat(d,(dept[d]||0).toFixed(2)+' hrs')).join('')+stat('Total',paid.toFixed(2)+' hrs');
     document.getElementById('lprTracked').innerHTML=stat('Room Cleaning',fmtMin(cleanM))+stat('Side Work',fmtMin(sideM))+stat('Rework / Corrections',fmtMin(reworkM))+stat('Reactive Maintenance',fmtMin(maintM))+stat('Room PM',fmtMin(pmM))+stat('Filter PM',fmtMin(filterM))+stat('Total RELAY Tracked',fmtMin(tracked));
     const rooms=new Set(finished.map(x=>String(x.room||'')).filter(Boolean)).size;document.getElementById('lprHousekeeping').innerHTML=stat('Rooms cleaned',rooms)+stat('Cleaning time',fmtMin(cleanM))+stat('Average clean time',rooms?Math.round(cleanM/rooms)+'m':'—')+stat('Inspection issues',issues.length)+stat('Rooms per paid HK hour',dept['Housekeeping']? (rooms/dept['Housekeeping']).toFixed(2):'—');
     document.getElementById('lprMaintenance').innerHTML=stat('Reactive tasks completed',maintDone.length)+stat('Room PM completed',pmHist.length)+stat('Filter PM completed',filterHist.length)+stat('Tracked maintenance time',fmtMin(maintM+pmM+filterM))+stat('Tasks per paid maintenance hour',dept['Maintenance']?((maintDone.length+pmHist.length+filterHist.length)/dept['Maintenance']).toFixed(2):'—');
     const unclassified=Math.max(0,paid*60-tracked);document.getElementById('lprVisibility').innerHTML=stat('Paid labor',paid?paid.toFixed(2)+' hrs':'Enter ADP hours')+stat('RELAY tracked activity',fmtMin(tracked))+stat('Unclassified time',paid?fmtMin(unclassified):'—')+stat('Time visible in RELAY',paid?visibility+'%':'—')+(paid?'<div class="lpr-track"><i style="width:'+Math.min(100,visibility)+'%"></i></div>':'');
     status.textContent=saved_.length?'Saved weekly hours loaded • '+w.start+' through '+w.end:'No ADP hours saved yet for '+w.start+' through '+w.end;
   }catch(e){status.textContent='Could not load Labor & Productivity: '+e.message;document.getElementById('lprHoursBody').innerHTML='<tr><td colspan="4">Backend update is required before this report can load.</td></tr>'}
 }
 async function save_(){const sid=localStorage.getItem('relaySessionId')||'',w=week(),status=document.getElementById('lprHoursStatus'),inputs=[...document.querySelectorAll('#lprHoursBody .lpr-hours')],rows=[];inputs.forEach(i=>{const u=users_.find(x=>String(x.userId)===String(i.dataset.user)),v=i.value.trim();if(u&&v!==''&&Number.isFinite(Number(v))&&Number(v)>=0)rows.push({userId:u.userId,employeeName:u.name,department:u.department,role:u.role,actualHours:Number(v)})});status.textContent='Saving weekly ADP hours…';const r=await apiPost({action:'saveLaborHours',sessionId:sid,weekStart:w.start,weekEnd:w.end,rows:rows},45000);if(!r.ok){status.textContent='Save failed: '+(r.reason||r.error||'Unknown error');return}status.textContent='✓ '+r.saved+' weekly labor records saved permanently.';await load_()}
 open.addEventListener('click',()=>{library.hidden=true;document.querySelectorAll('#reportsView>section').forEach(x=>x.hidden=true);report.hidden=false;if(!document.getElementById('lprWeekEnd').value)document.getElementById('lprWeekEnd').value=defaultSunday_();load_()});
 document.getElementById('backFromLaborProductivityReport').addEventListener('click',()=>{report.hidden=true;library.hidden=false});document.getElementById('printLaborProductivityReport').addEventListener('click',()=>window.print());document.getElementById('lprWeekEnd').addEventListener('change',load_);document.getElementById('lprSaveHours').addEventListener('click',save_);document.querySelectorAll('[data-view="Reports"]').forEach(b=>b.addEventListener('click',()=>{report.hidden=true}));
})();
