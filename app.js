...
async function loadInspectorPerformanceReport_(){
 const status=document.getElementById('iprStatus'),select=document.getElementById('iprInspector');if(!status||!select)return;const eligibleCurrent=(currentUser&&currentUser.roles&&currentUser.roles.some(role=>['ADMIN','INSPECTOR'].includes(role)))?[currentUser.name]:[];if(!select.value&&eligibleCurrent.length){select.innerHTML='<option value="'+dorEsc_(eligibleCurrent[0])+'">'+dorEsc_(eligibleCurrent[0])+'</option>';select.value=eligibleCurrent[0]}status.textContent='LOADING';const r=iprRange_();
 try{
  ...
  const reportNames=ins.map(iprInspectorName_).filter(Boolean),names=[...new Set([...reportNames,...eligibleCurrent])].sort(),old=select.value;select.innerHTML=names.length?names.map(n=>'<option value="'+dorEsc_(n)+'">'+dorEsc_(n)+'</option>').join(''):'<option value="">No inspectors available</option>';if(names.includes(old))select.value=old;else if(names.includes(currentUser?.name))select.value=currentUser.name;const name=select.value;if(!name){status.textContent='NO DATA';return}
