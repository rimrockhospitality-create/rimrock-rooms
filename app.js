...
async function loadDailyOperationsReport_(){
 ...
 document.getElementById('dorFrontDesk').innerHTML='<b>'+side.filter(x=>String(x.status).toUpperCase()==='COMPLETE').length+'</b> side work complete <span>•</span> <b>'+side.filter(x=>String(x.status).toUpperCase()!=='COMPLETE').length+'</b> outstanding<br><b>'+openNotes.length+'</b> open follow-up'+(openNotes.length===1?'':'s');
