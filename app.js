...
async function loadDailyOperationsReport_(){
 ...
 const readiness=assignments.length?Math.round(passed/assignments.length*100):0;document.getElementById('dorSummary').innerHTML=[dorMetric_('ASSIGNED',assignments.length),dorMetric_('CLEANED',complete),dorMetric_('PASSED',passed),dorMetric_('OUTSTANDING',Math.max(0,assignments.length-passed)),dorMetric_('MAINT. HOLDS',holds.length),dorMetric_('ROOM READY',assignments.length?readiness+'%':'—')].join('');set('dorStatus',assignments.length&&passed>=assignments.length?'DAY COMPLETE':readiness+'% ROOM READY');
 ...
 const firstSync=syncSorted.length?dorTime_(syncSorted[0].syncedAt||syncSorted[0].createdAt||syncSorted[0].timestamp):'—',lastSync=syncSorted.length?dorTime_(syncSorted.at(-1).syncedAt||syncSorted.at(-1).createdAt||syncSorted.at(-1).timestamp):'—',lastBy=syncSorted.length?(syncSorted.at(-1).syncedBy||syncSorted.at(-1).user||'—'):'—';set('dorSyncLine',syncSorted.length?syncSorted.length+' sync'+(syncSorted.length===1?'':'s')+' • First '+firstSync+' • Last '+lastSync+' • '+lastBy:'No Choice sync recorded');set('dorFooterFresh',syncSorted.length?'Choice current through '+lastSync:'Choice sync not recorded');
 ...
 document.getElementById('dorInspection').innerHTML=[dorMetric_('INSPECTED',ins.length),dorMetric_('PASSED',passed),dorMetric_('DEFICIENCIES',issues.length),dorMetric_('INSPECTOR FIXED',fixed),dorMetric_('SENT BACK',rework)].join('');
 document.getElementById('dorMaintenance').innerHTML=[dorMetric_('LOGGED',maint.length),dorMetric_('OPEN',maintOpen.length),dorMetric_('HOLDS',holds.length),dorMetric_('RESOLVED',maint.length-maintOpen.length)].join('');
 ...
 events.sort((a,b)=>new Date(a[0])-new Date(b[0]));document.getElementById('dorTimeline').innerHTML=events.length?events.slice(-6).map(e=>'<div><b>'+dorTime_(e[0])+'</b><span>'+dorEsc_(e[1])+'</span></div>').join(''):'<div><span>No significant activity recorded.</span></div>';const photoRefs=[];issues.filter(x=>x.photoRef).forEach(x=>photoRefs.push({ref:x.photoRef,label:'Room '+(x.room||'')+' • '+(x.description||'Inspection photo')}));maint.filter(x=>x.photoRef).forEach(x=>photoRefs.push({ref:x.photoRef,label:'Room '+(x.room||'')+' • Maintenance'}));const photoHost=document.getElementById('dorPhotos');photoHost.innerHTML=photoRefs.length?photoRefs.slice(0,4).map((x,i)=>'<div class="dor-photo" data-photo="'+dorEsc_(x.ref)+'"><div class="dor-photo-loading">PHOTO '+(i+1)+'</div><small>'+dorEsc_(x.label)+'</small></div>').join(''):'<div class="dor-photo-empty">Photos captured during inspections, maintenance and property walks will tell the story of the day here.</div>';for(const el of photoHost.querySelectorAll('.dor-photo')){try{const r=await apiPost({action:'getInspectionPhoto',photoRef:el.dataset.photo},45000);if(r.ok&&r.dataUrl)el.insertAdjacentHTML('afterbegin','<img src="'+r.dataUrl+'" alt="">')}catch(_){}}
}
