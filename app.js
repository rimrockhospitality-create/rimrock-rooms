...
async function loadDailyOperationsReport_(){
 ...
 const lfItems=(today.lostFound||today.lost_found||today.lostFoundItems||today.lost_found_items||[]),lfStatus=x=>String(x.status||'').toUpperCase().replace(/[^A-Z]+/g,'_'),logged=lfItems.filter(x=>dorDateKey_(x.businessDate||x.business_date||x.foundAt||x.found_at)===reportDay),due=lfItems.filter(x=>dorDateKey_(x.dispositionDate||x.disposition_date)===reportDay&&!['RETURNED','RETURNED_PICKED_UP','SHIPPED','DISPOSED','DISPOSED_DONATED'].includes(lfStatus(x)));
