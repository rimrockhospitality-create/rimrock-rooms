...
(function(){
 ...
 function render(board,warranty){
  ...
  const wo=history.reduce((n,x)=>n+Number(x.workOrders||x.work_orders||0),0),roomsWith=history.filter(x=>Number(x.workOrders||x.work_orders||0)>0).length;document.getElementById('pmrDeficiencies').innerHTML=[['Rooms PM’d',history.length],['Rooms with deficiencies',roomsWith],['Maintenance work orders generated',wo],['Warranty items in RELAY',(warranty||[]).filter(x=>{const d=key(x.businessDate||x.business_date||x.reportedAt||x.reported_at);return !d||inRange({businessDate:d},range,today)}).length]].map(x=>'<div class="pmr-stat-row"><span>'+x[0]+'</span><b>'+x[1]+'</b></div>').join('')+'<p>Deficiency category detail will populate as PM completion history exposes item-level results.</p>';
  ...
  const warr=(warranty||[]).filter(x=>{const d=key(x.businessDate||x.business_date||x.reportedAt||x.reported_at);return !d||inRange({businessDate:d},range,today)});document.getElementById('pmrFlow').innerHTML=[['PMs completed',history.length],['Deficiencies / work orders',wo],['Warranty items identified',warr.length]].map((x,i)=>'<div class="pmr-flow-line"><span>'+x[0]+'</span><strong>'+x[1]+'</strong></div>'+(i<2?'<div class="pmr-flow-arrow">↓</div>':'')).join('')+'<p>Replacement value protected will be added when value tracking is captured in RELAY.</p>';
 }
