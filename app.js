...
async function loadDailyOperationsReport_(){
 ...
 const hk={};completedSessions.forEach(x=>{const n=x.housekeeper||x.housekeeper_name||'Unassigned';hk[n]??={rooms:new Set(),min:0};if(x.room)hk[n].rooms.add(String(x.room));const a=new Date(x.startedAt||x.startTime||x.started_at||x.start_time),b=new Date(x.completedAt||x.endTime||x.completed_at||x.end_time||x.readyAt||x.ready_at);if(!isNaN(a)&&!isNaN(b))hk[n].min+=Math.max(0,(b-a)/60000)});
