...
async function loadDailyOperationsReport_(){
 ...
 const [today,work,lf,maintenanceReport,...shiftResults]=await Promise.all([apiPost({action:'getToday',businessDate:date},45000),apiPost({action:'getWorkBoard'},45000),apiPost({action:'getLostFound',sessionId:sid},45000),apiPost({action:'getMaintenanceReport',sessionId:sid,businessDate:date},45000).catch(()=>({ok:false})),apiPost({action:'getShiftOperations',sessionId:sid,businessDate:date},45000)]);
 ...
 const assignments=today.assignments||[],clean=today.cleaningSessions||[],ins=today.inspections||[],issues=today.inspectionIssues||[],maintToday=today.maintenance||today.maintenanceIssues||[],maintReportRows=maintenanceReport.maintenance||maintenanceReport.issues||maintenanceReport.maintenanceIssues||maintenanceReport.tickets||[],maintRaw=Array.isArray(maintReportRows)&&maintReportRows.length?maintReportRows:maintToday,maint=maintRaw,syncs=today.choiceSyncs||today.syncs||[],roomPhotos=today.roomPhotos||today.photos||[],propertyWalkPhotos=today.propertyWalkPhotos||[];
