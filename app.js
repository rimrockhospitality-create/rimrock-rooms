...
async function loadDailyOperationsReport_(){
 ...
 const [today,work,lf,maintenanceReport,maintenanceBoard,pmBoard,...shiftResults]=await Promise.all([apiPost({action:'getToday',businessDate:date},45000),apiPost({action:'getWorkBoard'},45000),apiPost({action:'getLostFound',sessionId:sid},45000),apiPost({action:'getMaintenanceReport',sessionId:sid,businessDate:date},45000).catch(()=>({ok:false})),apiPost({action:'getMaintenanceBoard',sessionId:sid,businessDate:date,worker:currentUser.name},45000).catch(()=>({ok:false})),apiPost({action:'getPmBoard',sessionId:sid},45000).catch(()=>({ok:false})),apiPost({action:'getShiftOperations',sessionId:sid,businessDate:date},45000)]);
 ...
 if(!maintenanceReport.ok&&maintToday.length===0)document.getElementById('dorMaintenance').insertAdjacentHTML('beforeend','<span class="dor-pm-checklist">PM detail unavailable; maintenance fallback loaded from daily state.</span>'); if(maint.length===0){const dbg=document.createElement('div');dbg.style.cssText='font-size:10px;color:#8aa3b5;margin-top:4px;word-break:break-word';dbg.textContent='MAINT DIAG • report='+reportDay+' • board='+(maintenanceBoard&&maintenanceBoard.ok?'OK':'FAIL')+' • boardIssues='+maintBoardRows.length+' • reportRows='+maintReportRows.length+' • todayRows='+maintToday.length+' • boardSample='+(maintBoardRows[0]?JSON.stringify({date:maintBoardRows[0].businessDate||maintBoardRows[0].business_date,room:maintBoardRows[0].room,status:maintBoardRows[0].status,id:maintBoardRows[0].maintenance_id}):'none');document.getElementById('dorMaintenance').appendChild(dbg);}
