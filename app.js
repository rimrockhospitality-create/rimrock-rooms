...
async function scanMaintenanceQr_(maintenanceId,room,button){
 const panel=document.getElementById('maintenanceQrPanel'),msg=document.getElementById('maintenanceQrMessage'),title=document.getElementById('maintenanceQrTitle'),box=document.getElementById('maintenanceQrCamera'),isRoom=/^\d{3}$/.test(String(room)),label=isRoom?'Room '+room:String(room);title.textContent=label;msg.textContent='Opening camera…';box.innerHTML='▦';panel.hidden=false;
 try{maintenanceQrStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}},audio:false});const video=document.createElement('video'),canvas=document.createElement('canvas'),ctx=canvas.getContext('2d',{willReadFrequently:true});video.setAttribute('playsinline','');video.muted=true;video.autoplay=true;video.srcObject=maintenanceQrStream;box.innerHTML='';box.appendChild(video);video.style.width='100%';video.style.height='100%';video.style.objectFit='cover';video.style.borderRadius='12px';await video.play();msg.textContent='Scan the '+label+' QR. Scanning starts the maintenance work session automatically.';const detector=('BarcodeDetector' in window)?new BarcodeDetector({formats:['qr_code']}):null,expected=isRoom?'CO534-RM-'+room:'CO534-LOC-'+String(room).toUpperCase().replace(/[^A-Z0-9]+/g,'-').replace(/^-|-$/g,'');
