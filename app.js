...
async function loadMaintenanceBoard(){
  ...
  try{
    ...
    if(!state){const isMaintOnly=(currentUser?.roles||[]).includes('MAINTENANCE')&&!(currentUser?.roles||[]).some(r=>['ADMIN','INSPECTOR','FRONT DESK'].includes(r));if(isMaintOnly){const targeted=await apiPost({action:'getMaintenanceBoard',sessionId:localStorage.getItem('relaySessionId'),businessDate:housekeepingBusinessDate(),worker:currentUser.name},20000);if(!targeted?.ok)throw new Error(targeted?.reason||targeted?.error||'Could not load maintenance');state=targeted}else state=await apiPost({action:'getToday',businessDate:housekeepingBusinessDate()},45000);relayCacheSet_(key,state)}
