...
async function loadDailyOperationsReport_(){
 ...
 document.getElementById('dorLostFound').innerHTML=((!logged.length&&!due.length)?'<b>0</b> logged today <span>•</span> <b>0</b> due':logged.slice(0,2).map(x=>'<div><b>'+dorEsc_(x.id)+'</b> '+dorEsc_(x.description)+'</div>').concat(due.slice(0,2).map(x=>'<div class="dor-alert"><b>ACTION REQUIRED</b> '+dorEsc_(x.id)+' '+dorEsc_(x.description)+'</div>')).join(''))+'<div style="margin-top:8px;padding:7px 9px;border:1px dashed #8a6d1d;background:#fff9df;font-size:10px;line-height:1.35"><b>L&F DIAGNOSTIC</b> • request '+dorEsc_(date)+' • API '+(lostFoundReport?.ok?'OK':'FAIL')+' • rows '+dorEsc_(lfDiag.totalRows??'—')+' • logged '+logged.length+' • due '+due.length+' • active '+dorEsc_(lfDiag.activeBusinessDate??'—')+'<br>'+dorEsc_((lfDiag.seen||[]).map(x=>x.id+' ['+x.idDay+' / '+x.foundDay+']').join(' | ')||lostFoundReport?.error||lostFoundReport?.reason||'No rows returned')+'</div>';
