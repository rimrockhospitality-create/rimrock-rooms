...
async function loadDailyOperationsReport_(){
 ...
 const assignments=today.assignments||[],clean=today.cleaningSessions||[],ins=today.inspections||[],issues=today.inspectionIssues||[],maint=today.maintenance||today.maintenanceIssues||[],syncs=today.choiceSyncs||today.syncs||[],roomPhotos=today.roomPhotos||today.photos||[],propertyWalkPhotos=today.propertyWalkPhotos||[];
 ...
 const hk={};completedSessions.forEach(x=>{const n=x.housekeeper||'Unassigned';hk[n]??={rooms:new Set(),min:0};if(x.room)hk[n].rooms.add(String(x.room));const a=new Date(x.startedAt||x.startTime),b=new Date(x.completedAt||x.endTime);if(!isNaN(a)&&!isNaN(b))hk[n].min+=Math.max(0,(b-a)/60000)});
 document.getElementById('dorHousekeeping').innerHTML=Object.keys(hk).length?'<table><thead><tr><th>ASSOCIATE</th><th>ROOMS</th><th>ACTIVE CLEANING</th><th>AVG / ROOM</th><th>DEFICIENCIES</th></tr></thead><tbody>'+Object.entries(hk).map(([n,x])=>'<tr><td>'+dorEsc_(n)+'</td><td>'+x.rooms.size+'</td><td>'+Math.round(x.min)+' min</td><td>'+(x.rooms.size?Math.round(x.min/x.rooms.size):0)+' min</td><td>'+issues.filter(i=>String(i.housekeeper||'')===n).length+'</td></tr>').join('')+'</tbody></table>':'<p>No housekeeping production recorded.</p>';
 ...
 const sideAll=work.ok?(work.sideWork||[]):[],side=sideAll.filter(x=>!x.businessDate||dorDateKey_(x.businessDate)===date),shiftNotes=shift.ok?(shift.openNotes||shift.shiftNotes||[]):[],openNotes=shiftNotes.filter(x=>!['RESOLVED','CLOSED','COMPLETE'].includes(String(x.status||'OPEN').toUpperCase()));
 const noteTime=x=>x.createdAt||x.created_at||x.noteAt||x.timestamp||x.updatedAt||'',noteText=x=>x.note||x.text||x.message||x.description||x.issue||'',noteShift=x=>String(x.shift||x.shiftName||x.type||'PASS-ON').replace(/_/g,' '),noteBy=x=>x.createdBy||x.enteredBy||x.author||x.user||x.actor||'';const dailyNotes=shiftNotes.filter(x=>!x.businessDate||dorDateKey_(x.businessDate)===date).sort((a,b)=>new Date(noteTime(a))-new Date(noteTime(b)));document.getElementById('dorFrontDesk').innerHTML=(dailyNotes.length?dailyNotes.slice(-5).map(x=>'<div class="dor3-passon-note"><b>'+dorEsc_(noteShift(x))+'</b><span>'+dorEsc_(noteText(x)||'Shift note recorded')+'</span><small>'+dorEsc_(noteBy(x))+(noteTime(x)?' • '+dorTime_(noteTime(x)):'')+'</small></div>').join(''):'<div class="dor3-passon-empty">No Front Desk pass-on notes recorded today.</div>')+'<div class="dor3-fd-foot">'+side.filter(x=>String(x.status).toUpperCase()==='COMPLETE').length+' side work complete • '+side.filter(x=>String(x.status).toUpperCase()!=='COMPLETE').length+' outstanding • '+openNotes.length+' open follow-up'+(openNotes.length===1?'':'s')+'</div>';
 const lfItems=lf.ok?(lf.items||[]):[],lfStatus=x=>String(x.status||'').toUpperCase().replace(/[^A-Z]+/g,'_'),logged=lfItems.filter(x=>dorDateKey_(x.foundAt)===date),due=lfItems.filter(x=>dorDateKey_(x.dispositionDate)===date&&!['RETURNED','RETURNED_PICKED_UP','SHIPPED','DISPOSED','DISPOSED_DONATED'].includes(lfStatus(x)));
