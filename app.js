...
async function loadDailyOperationsReport_(){
 ...
 const lfItems=lf.ok?(lf.items||[]):[],lfStatus=x=>String(x.status||'').toUpperCase().replace(/[^A-Z]+/g,'_'),lfBusinessDate=x=>{const explicit=x.businessDate||x.business_date||'';if(explicit)return dorDateKey_(explicit);const m=String(x.id||'').match(/^LF-(\d{4})(\d{2})(\d{2})-/);return m?m[1]+'-'+m[2]+'-'+m[3]:dorDateKey_(x.foundAt)},logged=lfItems.filter(x=>lfBusinessDate(x)===date),due=lfItems.filter(x=>dorDateKey_(x.dispositionDate)===date&&!['RETURNED','RETURNED_PICKED_UP','SHIPPED','DISPOSED','DISPOSED_DONATED'].includes(lfStatus(x)));
