...
async function loadDailyOperationsReport_(){
 ...
 const [today,work,lf,maintenanceReport,pmBoard,...shiftResults]=await Promise.all([apiPost({action:'getToday',businessDate:date},45000),apiPost({action:'getWorkBoard'},45000),apiPost({action:'getLostFound',sessionId:sid},45000),apiPost({action:'getMaintenanceReport',sessionId:sid,businessDate:date},45000).catch(()=>({ok:false})),apiPost({action:'getPmBoard',sessionId:sid},45000).catch(()=>({ok:false})),apiPost({action:'getShiftOperations',sessionId:sid,businessDate:date},45000)]);
 ...
 const reportDay=dorDateKey_(date),assignments=today.assignments||[],clean=today.cleaningSessions||[],ins=today.inspections||[],issues=today.inspectionIssues||[],maintToday=today.maintenance||today.maintenanceIssues||[],maintReportRows=maintenanceReport.maintenance||maintenanceReport.issues||maintenanceReport.maintenanceIssues||maintenanceReport.tickets||[],maintRaw=(Array.isArray(maintReportRows)&&maintReportRows.length?maintReportRows:maintToday).filter(x=>!x.businessDate&&!x.business_date||dorDateKey_(x.businessDate||x.business_date)===reportDay),maint=maintRaw,pmSource=maintenanceReport.pmCompletions||maintenanceReport.pm||maintenanceReport.completedPm||((pmBoard&&pmBoard.ok)?pmBoard.history:[])||[],pmRows=pmSource.filter(x=>dorDateKey_(x.businessDate||x.business_date)===reportDay),syncs=today.choiceSyncs||today.syncs||[],roomPhotos=today.roomPhotos||today.photos||[],propertyWalkPhotos=today.propertyWalkPhotos||[];
 ...
 document.getElementById('dorMaintenance').innerHTML=[dorMetric_('Issues Logged',maint.filter(x=>dorDateKey_(x.business_date)===dorDateKey_(date)).length),dorMetric_('Resolved',maintResolved.length),dorMetric_('Still Open',maintOpen.length),dorMetric_('PM Completed',pmRows.length),dorMetric_('PM Active Min',pmRows.reduce((n,x)=>n+Number(x.activeMinutes||x.active_minutes||0),0).toFixed(2)),dorMetric_('Repair Active Min',(maintenanceReport.work||maintenanceReport.workSessions||maintenanceReport.sessions||[]).reduce((n,x)=>n+Number(x.activeMinutes||x.active_minutes||0),0).toFixed(2))].join('');
 if(!maintenanceReport.ok&&maintToday.length===0)document.getElementById('dorMaintenance').insertAdjacentHTML('beforeend','<span class="dor-pm-checklist">PM detail unavailable; maintenance fallback loaded from daily state.</span>'); if(pmRows.length===0||maint.length===0){const dbg=document.createElement('div');dbg.style.cssText='font-size:10px;color:#8aa3b5;margin-top:4px;word-break:break-word';dbg.textContent='DIAG • reportDay='+reportDay+' • maintReport='+(maintenanceReport.ok?'OK':'FAIL '+(maintenanceReport.error||maintenanceReport.reason||''))+' • maintRows='+maintReportRows.length+' • todayMaint='+maintToday.length+' • pmBoard='+(pmBoard&&pmBoard.ok?'OK':'FAIL '+((pmBoard&&pmBoard.error)||(pmBoard&&pmBoard.reason)||''))+' • pmHistory='+((pmBoard&&pmBoard.history)||[]).length+' • pmDates='+((pmBoard&&pmBoard.history)||[]).slice(-3).map(x=>(x.businessDate||x.business_date||'?')+':'+(x.room||'?')).join(',');document.getElementById('dorMaintenance').appendChild(dbg);}
